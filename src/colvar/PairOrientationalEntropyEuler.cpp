...
class PairOrientationalEntropyEuler : public Colvar {
  ...
  Matrix<double> referenceGofrAngle1;
  Matrix<double> referenceGofrAngle2;
  Matrix<double> referenceGofrAngle3;
  double epsilon;
  double densityReference;
  // Average gofr
  Matrix<double> avgGofrAngle1;
  Matrix<double> avgGofrAngle2;
  Matrix<double> avgGofrAngle3;
  unsigned iteration;
  bool doAverageGofr;
  ...
  bool doLowComm;
public:
  explicit PairOrientationalEntropyEuler(const ActionOptions&);
  ~PairOrientationalEntropyEuler();
  virtual void calculate();
  virtual void prepare();
  static void registerKeywords( Keywords& keys );
};

PLUMED_REGISTER_ACTION(PairOrientationalEntropyEuler,"PAIR_ORIENTATIONAL_ENTROPY_EULER")

void PairOrientationalEntropyEuler::registerKeywords( Keywords& keys ){
  ...
  keys.add("optional","REFERENCE_GOFR_FNAMES","The names of the files with the reference g(r)s");
...
PairOrientationalEntropyEuler::PairOrientationalEntropyEuler(const ActionOptions&ao):
...
// neighbor list stuff
  ...
  std::string referenceGofrFileNamesString;
  parse("REFERENCE_GOFR_FNAMES",referenceGofrFileNamesString); 
  if (!referenceGofrFileNameString.empty() ) {
    epsilon=1.e-8;
    log.printf("  Reading a reference g(r) from the file %s . \n", referenceGofrFileName.c_str() );
    doReferenceGofr=true;
    referenceGofrAngle1.resize(nhist_[0],nhist_[1]);
    referenceGofrAngle2.resize(nhist_[0],nhist_[1]);
    referenceGofrAngle3.resize(nhist_[0],nhist_[1]);
    // Separate into different strings
    std::stringstream ss(referenceGofrFileNamesString);
    std::vector<std::string> referenceGofrFileNames;
    while( ss.good() )
    {
        std::string substr;
        std::getline( ss, substr, ',' );
        referenceGofrFileNames.push_back( substr );
    }
    IFile ifile1, ifile2, ifile3; 
    ifile1.link(*this);
    ifile2.link(*this);
    ifile3.link(*this);
    ifile1.open(referenceGofrFileName[0]);
    ifile2.open(referenceGofrFileName[1]);
    ifile3.open(referenceGofrFileName[2]);
    for(unsigned int i=0;i<nhist_[0];i++) {
       for(unsigned int j=0;j<nhist_[1];j++) {
       double tmp_r, tmp_theta;
       ifile1.scanField("r",tmp_r).scanField("theta",tmp_theta).scanField("gofr",referenceGofrAngle1[i][j]).scanField();
       ifile2.scanField("r",tmp_r).scanField("theta",tmp_theta).scanField("gofr",referenceGofrAngle2[i][j]).scanField();
       ifile3.scanField("r",tmp_r).scanField("theta",tmp_theta).scanField("gofr",referenceGofrAngle3[i][j]).scanField();
       }
    }
  }
  ...
  if (doAverageGofr) {
     iteration = 1;
     avgGofrAngle1.resize(nhist_[0],nhist_[1]);
     avgGofrAngle2.resize(nhist_[0],nhist_[1]);
     avgGofrAngle3.resize(nhist_[0],nhist_[1]);
  }
  averageGofrTau=0;
...
PairOrientationalEntropyEuler::~PairOrientationalEntropyEuler(){
  if (doneigh) {
     nl->printStats();
     delete nl;
  }
...
void PairOrientationalEntropyEuler::prepare(){
...
// calculator
void PairOrientationalEntropyEuler::calculate()
{
...
double PairOrientationalEntropyEuler::kernel(vector<double> distance, double invNormKernel, vector<double>&der)const{
...
double PairOrientationalEntropyEuler::integrate(Matrix<double> integrand, vector<double> delta)const{
  // Trapezoid rule
  double result = 0.;
...
Vector PairOrientationalEntropyEuler::integrate(Matrix<Vector> integrand, vector<double> delta)const{
  // Trapezoid rule
  Vector result;
  for(unsigned i=1;i<(nhist_[0]-1);++i){
     for(unsigned j=1;j<(nhist_[1]-1);++j){
...
Tensor PairOrientationalEntropyEuler::integrate(Matrix<Tensor> integrand, vector<double> delta)const{
  // Trapezoid rule
  Tensor result;
  for(unsigned i=1;i<(nhist_[0]-1);++i){
     for(unsigned j=1;j<(nhist_[1]-1);++j){
...
void PairOrientationalEntropyEuler::outputGofr(Matrix<double> gofr, const char* fileName) {
  for(unsigned i=0;i<nhist_[0];++i){
     for(unsigned j=0;j<nhist_[1];++j){
