...
class PairEntropy : public Colvar {
  ...
  double CombinePerAtomValues(vector<double> entropies,Matrix<Vector> derivativesPerAtom, vector<Tensor> virialPerAtom, vector<Vector> &deriv, Tensor &virial)const;
public:
...
// neighbor list stuff
  ...
  deltar=maxr/nhist;
  deltaBin = std::floor(3*sigma/deltar);
}
...
void PairEntropy::calculate()
{
  ...
  Matrix<Vector> derivativesPerAtom(getNumberOfAtoms(),getNumberOfAtoms());
  vector<Tensor> virialPerAtom(getNumberOfAtoms());
  ...
  for(unsigned int i=rank;i<getNumberOfAtoms();i+=stride) {   
    ...
    entropies[i] = -2*pi*density*integrate(integrand,deltar); 
    ...
    for(unsigned j=0;j<getNumberOfAtoms();++j) {
      ...
      // Integrate
      derivativesPerAtom[i][j] = -2*pi*density*integrate(integrandDerivatives,deltar);
    }
    // Virial of positions
    ...
    // Integrate virial
    virialPerAtom[i] = -2*pi*density*integrate(integrandVirial,deltar);
    // Virial of volume
    // Construct virial integrand
    ...
    // Integrate virial
    virialPerAtom[i] += -2*pi*density*integrate(integrandVirialVolume,deltar)*Tensor::identity();
  } 
  if(!serial){
    comm.Sum(&entropies[0],getNumberOfAtoms());
    comm.Sum(&derivativesPerAtom[0][0][0],getNumberOfAtoms()*getNumberOfAtoms()*3);
    comm.Sum(&virialPerAtom[0][0][0],getNumberOfAtoms()*3*3);
  }
  vector<Vector> deriv(getNumberOfAtoms());
  Tensor virial;
  double CVvalue = CombinePerAtomValues(entropies, derivativesPerAtom, virialPerAtom, deriv, virial);
...
double CombinePerAtomValues(vector<double> entropies,Matrix<Vector> derivativesPerAtom, vector<Tensor> virialPerAtom, vector<Vector> &deriv, Tensor &virial) {
  // Sum function
  double result;
  for (unsigned int i=0;i<entropies.size();++i) result += entropies[i];
  for (unsigned int i=0;i<entropies.size();++i) {
    for (unsigned int j=0;j<entropies.size();++j) {
      deriv[i] += derivativesPerAtom[j][i];
    }
  }
  for (unsigned int i=0; i<entropies.size();++i) virial += virialPerAtom[i];
  return result;
}
